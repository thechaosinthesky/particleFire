...
exports.sessionDeviceKey = function(req){
...
exports.updateSessionKey = function (req, user, callback) {
  Users.findAndModify({
    "query": {'_id': user._id},
    "update": {$set: {'sessionDeviceKey': req.ip + '-' + req.headers['user-agent']}}
  },
  ...
  if(valid){
    ...
    if(req.user.admin || user_device_id){
      ...
      IOs.findAndModify({
            ...
            "update": { 
                "$set": {"name": params.name, "type": params.type, "device_id": params.device_id, "settings": params.settings}
            }
        },
